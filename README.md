...
This is a vibe-coded fix to get classic Touhou games (**Touhou 6, 7, and 8**) running on Windows 11 ARM64 PC's.

It assumes you already have a build for x86-64 with the fan patch installed. This is a drag and drop fix with a set of files and an automated one-stop installer script.
...
## ⚡ One-Stop-Shop Automated Installer (`Install-Touhou-Fix.ps1`)

Prefer a single script to do everything automatically?
...
3. The script automatically:
   ...
   - Downloads, installs & configures `thcrap` English translation patch for all detected games
...
4. **Launch and play**: Double-click `th06 (thpatch-en).exe` / run `thcrap\bin\thcrap_loader.exe thpatch-en.js <game>`!

---

## 🛠️ Manual Setup Guide: From CD Rip to Playable

If you prefer doing it manually:

### 📥 Step 1: Obtain Game Files (CD Rip / Base Game)
...
1. Download **THCRAP** from [THPatch (thpatch.net)](https://www.thpatch.net/wiki/Touhou_Patch_Center).
2. Extract `thcrap` into your Touhou game folder.
3. Run `thcrap_configure.exe`, select **English (`lang_en`)**, and let it detect your game executable.

### 🚀 Step 3: Apply This Fix Pack (Drag & Drop)
1. **Download this repo** (click **Code -> Download ZIP**).
2. **Copy and replace** all files into your Touhou game folder.
3. **Launch and play!**
...
|---|---|---|
| `Install-Touhou-Fix.ps1` | Custom automated PowerShell installer script. | One-stop-shop script that auto-detects TH06, TH07, and TH08, downloads Crosire `d3d8to9`, Microsoft `d3dx9_43.dll`, installs `thcrap`, configures English translation patches, and sets up windowed mode & 60 FPS VPatch automatically. Skips downloading components if already present. |
| `d3d8.dll` | Downloaded from Crosire's official `d3d8to9` GitHub release (`crosire/d3d8to9` v1.15.1). | Placed in the game folder as `d3d8.dll` to translate legacy DirectX 8 calls to Direct3D 9 / Direct3D 12. Windows 11 ARM64 drivers fail on native DirectX 8 calls; translating them to D3D9/D3D12 enables hardware acceleration on Qualcomm Adreno GPUs for TH06, TH07, and TH08. |
