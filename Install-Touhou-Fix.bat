<# :
@echo off
title Universal Touhou Patching Script
cd /d "%~dp0"
...
# 56-byte cfg for TH06, TH07, TH08, TH09, TH095 (Byte 0x02 = 0x01 Windowed)
$cfg56Bytes = [byte[]](
...
# 52-byte cfg for TH10 through TH20 (Byte 0x06 = 0x01 Windowed)
$cfg52Bytes = [byte[]](
...
foreach ($g in $detectedGames) {
    ...
    if (-not (Test-Path -Path $cfgPath -ErrorAction SilentlyContinue) -or $ForceReinstall) {
        if ($g -in @("th06", "th07", "th08", "th09", "th095")) {
